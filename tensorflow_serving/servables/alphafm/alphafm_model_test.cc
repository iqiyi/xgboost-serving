...
namespace serving {
namespace {
TEST(AlphaFmModelTest, Basic) {
  AlphaFmModel alphafm_model(
  ...
  *(leaf_mapping_feature_score_2->mutable_score()) = {
  ...
  alphafm_model.Predict(fm_feature_score_vector, xgboost_feature_score_vector,
                        leaf_mapping_feature_score_vector, result);
  EXPECT_THAT(std::to_string(result[0]).substr(0, 6), "0.0678");
  EXPECT_THAT(std::to_string(result[1]).substr(0, 6), "0.0678");
}
} // namespace
} // namespace serving
} // namespace tensorflow
