...
#include "tensorflow_serving/servables/alphafm/alphafm_source_adapter.h"
...
namespace serving {
namespace {
TEST(AlphafmSourceAdapterTest, Basic) {
  ...
  *servable_0->mutable_servable_version_policy() = spolicy;
  ...
  auto adapter =
      std::unique_ptr<AlphafmSourceAdapter>(new AlphafmSourceAdapter());
