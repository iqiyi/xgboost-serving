...
bvar::LatencyRecorder AlphafmPredictor::alphafm_xgboost_latency_recorder(
    "alphafm_xgboost_predict");
bvar::LatencyRecorder
    AlphafmPredictor::alphafm_latency_recorder("alphafm_predict");

AlphafmPredictor::AlphafmPredictor() {}

Status AlphafmPredictor::Predict(ServerCore *core,
...
Status AlphafmPredictor::PredictWithModelSpec(ServerCore *core,
                                              const ModelSpec &model_spec,
                                              const PredictRequest &request,
                                              PredictResponse *response) {
  ServableHandle<AlphafmBundle> bundle;
  ...
  alphafm_xgboost_latency_recorder << elapsed_seconds.count() * 1000000;
  if (0 != status) {
  ...
  auto alphafm_model_handle = bundle->GetAlphafmModelHandle();
  std::vector<float> fm_result;
  start = std::chrono::system_clock::now();
  Status fm_status = alphafm_model_handle->Predict(
  ...
  alphafm_latency_recorder << elapsed_seconds.count() * 1000000;
  if (!fm_status.ok()) {
    LOG(ERROR) << "predict with Alphafm failed";
    return tensorflow::Status(tensorflow::error::UNKNOWN,
                              "predict with Alphafm failed");
  }
  Tensor output_tensor{DT_FLOAT, {fm_result.size()}};
  std::copy_n(fm_result.data(), output_tensor.flat<float>().size(),
