...
#include "tensorflow_serving/servables/alphafm_softmax/alphafm_softmax_source_adapter.h"
...
namespace serving {
namespace {
TEST(AlphafmSoftmaxSourceAdapterTest, Basic) {
  ...
  *servable_0->mutable_servable_version_policy() = spolicy;
  ...
  auto adapter = std::unique_ptr<AlphafmSoftmaxSourceAdapter>(
      new AlphafmSoftmaxSourceAdapter());
