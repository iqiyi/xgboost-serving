...
protected:
  static void SetUpTestCase() {
    TF_ASSERT_OK(CreateServerCore(test_util::GetCWD() +
                                      "/tensorflow_serving/servables/"
                                      "alphafm_softmax/testdata/test_model/",
                                  &server_core_));
  }
  ...
  static Status CreateServerCore(const string &model_path,
    ...
    model_config->set_model_platform("alphafm_softmax");
    ...
    options.platform_config_map =
        tensorflow::serving::CreateAlphafmSoftmaxPlatformConfigMap();
...
TEST_F(PredictImplTest, MissingOrEmptyModelSpec) {
  ...
  AlphafmSoftmaxPredictor predictor;
...
TEST_F(PredictImplTest, PredictionSuccess) {
  ...
  EXPECT_THAT(6, output_tensor_proto.float_val().size());
  EXPECT_THAT(
      "0.1879",
