...
size_t StringPiece::find_first_of(const char *s, size_t pos) const {
  const char *const end = ptr_ + length_;
  const char *const p = std::find_first_of(ptr_ + pos, end, s, s + strlen(s));

  return p == end ? std::string::npos : p - ptr_;
}

size_t StringPiece::find_first_not_of(const char *s, size_t pos) const {
  const char *const end = s + strlen(s);
  for (int i = pos; i < length_; ++i) {
    if (std::find(s, end, ptr_[i]) == end) {
      return i;
    }
  }
